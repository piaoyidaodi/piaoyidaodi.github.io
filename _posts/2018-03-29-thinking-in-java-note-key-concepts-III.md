...
tag: Core-Java
---
> `java`中一些重要的概念，**集合、异常、字符串、类型信息、泛型**等。
...
Class对象是用来创建类的所有常规对象的，Java使用它来执行RTTI。每编译一个新类，就会产生一个Class对象，被保存在同名的.class文件中。为了生成这个类的对象，JVM使用类加载器加载。

使用RTTI必须获得Class对象的引用：
- `Class.forName()`不需要为了获得Class引用而持有该类型的对象。
- `getClass()`可以获取感兴趣类型的Class引用。
- 使用类字面常量生成对Class对象的引用，如`int.class`，因为会在编译器受到检查，因此更加安全高效。

对于**static final**值是编译期常量的，可以不经初始化就可以读取；对于**只有static**的域，在访问前先进行链接（为该域分配存储空间）和初始化（初始化该存储空间）。

泛化的Class<T>可以提供编译期类型检查。关键字`instanceof`返回对象是否是某个类型的实例，且只可将其与命名类型比较，而不能与Class对象作比较。`Class.isInstance`提供了一种动态测试对象的途径。

instanceof和isInstance()的结果一样，==比较实际的Class对象而不考虑继承。

### 4.2 反射：运行时

代理：是为了提供额外的不同的操作，而插入的用来代替实际对象的对象。这些操作通常涉及实际对象，因此代理通常充当中间人角色。

动态代理：动态的创建代理并动态处理对所代理方法的调用。
- 在动态代理上的所有调用都会被重定向到单一的调用处理器上，因此通常会向调用处理器的构造器传递一个实际对象引用，从而使调用处理器在执行其中介任务时，可以将请求转发。

空对象：
- 它可以接受传递给它的所代表的对象的消息，但是将返回表示为实际并不存在任何“真实对”象的值。
- 空对象更加靠近数据，表示问题空间内的实体。

### 4.3 总结

RTTI允许通过匿名基类的引用来发现类型信息。面向对象语言的目的就是让我们在凡是可以使用的地方使用多态，只在必要时使用RTTI。

## 5. 泛型

面向对象语言中，多态算是一种泛化机制。泛型实现了参数化类型的概念。

Java泛型的核心概念：**告诉编译器想使用什么类型，然后编译器帮你处理一切细节**。

泛型的主要目的：
- 用来指定容器要持有什么类型的对象，并由编译器保证类型的正确性。
