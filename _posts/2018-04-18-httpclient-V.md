---
layout: post
title: "Apache HttpClient V"
categories: Log4j2
tag: Java-Libs
---
> `Apache HttpClient 4.5.5 Tutorial - V`

## 官方示例

### 响应处理

此示例演示如何使用响应处理程序处理HTTP响应。这是执行HTTP请求和处理HTTP响应的推荐方式。这种方法使调用者能够专注于处理HTTP响应的过程，并将系统资源释放的任务委托给HttpClient。使用HTTP响应处理程序可以保证底层HTTP连接在所有情况下都会自动释放回连接管理器。
...
此示例演示如何确保在手动处理HTTP响应的情况下将底层HTTP连接释放回连接管理器。
...
本示例演示如何定制和配置HTTP请求执行和连接管理的最常见方面。
...
本示例使用HttpClient针对需要用户身份验证的目标站点执行HTTP请求。
...
一个简单的示例，显示通过安全连接执行HTTP请求的隧道通过身份验证代理。
...
此示例演示如何使用HttpClient执行基于form的登录。
...
此示例显示如何定制HttpClient并以BASIC方案预先进行身份验证。通常，抢先认证可能被认为安全性较低，因此不鼓励。
...
此示例展示了如何定制HttpClient并以DIGEST方案预先进行身份验证。通常，抢先认证可能被认为安全性较低，因此不鼓励。
...
此示例说明如何使用ProxyClient为任意协议通过HTTP代理建立隧道。
...
### 本地Windows Negotiate/NTLM

此示例说明如何在Windows操作系统上运行时使用本地WindowsNegotiate/NTLM身份验证。
